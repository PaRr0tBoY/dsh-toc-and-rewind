 ...
 */
const EMPTY_ENTRIES = [];
const COLD_VIEW = Object.freeze({ status: 'cold', entries: EMPTY_ENTRIES });
 ...
 */
export function summarize(node, maxLength = SUMMARY_MAX_LENGTH) {
    let text = '';
...
export const TICK_WIDTH_RANGE = { min: 16, max: 32 };
/**
 ...
 */
export function tickWidthFor(length, maxLength, min = TICK_WIDTH_RANGE.min, max = TICK_WIDTH_RANGE.max) {
 ...
 */
export function extractUserEntries(snapshot) {
    const nodes = [];
    ...
    for (const node of snapshot.chat.nodes.values()) {
        if (node.kind === 'user' && node.visibility !== 'hidden')
            nodes.push(node);
    }
...
const EMPTY_DERIVED = Object.freeze({
    shadowedSeqs: Object.freeze(new Set()),
    nodesByKey: Object.freeze(new Map()),
});
/**
 ...
 */
export function deriveTocState(snapshot) {
 ...
 */
export function activeUserKey(rows, viewportTop, viewportBottom) {
...
export class TocController {
    ...
    publishFrom(snapshot) {
        ...
        this.view = Object.freeze({ status: 'ready', entries: extractUserEntries(snapshot) });
