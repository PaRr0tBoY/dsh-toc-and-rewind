...
export const InputBridgeView = memo(function InputBridgeView({ inputActions, bridge, }) {
    useEffect(() => {
...
export function registerInputBridge(ctx, bridge) {
    ctx.slots.inject('conversation.composer.dock', () => ctx.slots.register({ name: 'conversation.composer.dock', id: 'toc-tail-input', locale: 'toc-tail' }, (props) => (_jsx(InputBridgeView, { ...props, bridge: bridge }))));
}
