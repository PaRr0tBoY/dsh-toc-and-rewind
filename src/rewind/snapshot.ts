...
async function collectWorkspace(cwd: string): Promise<Record<string, string>> {
...
export class SnapshotStore {
   ...
   */
  async capture(session: Session, seq: number, cwd?: string): Promise<CaptureResult> {
    ...
    const payload: SnapshotPayload = { files: await collectWorkspace(resolve(workspace)) }
   ...
   */
  async restore(session: Session, targetSeq: number, cwd?: string): Promise<RestoreResult> {
    const workspace = cwd ?? session.header.cwd
    if (workspace === undefined) {
    ...
    const payload = JSON.parse(await readFile(join(dir, `${snapshotSeq}.json`), 'utf8')) as SnapshotPayload
    let restoredCount = 0
    const root = resolve(workspace)
    for (const [rel, content] of Object.entries(payload.files)) {
      const target = resolve(root, ...rel.split('/'))
      if (!target.startsWith(root + sep) && target !== root) continue // path traversal guard
