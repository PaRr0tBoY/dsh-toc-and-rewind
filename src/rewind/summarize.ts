...
export const TOC_REWIND_SOURCE = Object.freeze({ kind: 'plugin', plugin: 'toc-tail' } as const)
...
/** The resolved model target for the auxiliary call. */
 ...
 */
function resolveTarget(agent: Agent): ModelTarget {
  ...
  throw new Error(
    'toc-tail rewind: no provider/model available for summarization — route one request or set AgentOptions fields',
  )
}
