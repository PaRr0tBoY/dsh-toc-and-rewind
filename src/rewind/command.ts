...
import { executeRewind } from './fold.ts'
...
export function parseRewindArgs(rawInput: string): { seq: number; options: { code: boolean; summary: boolean } } {
...
export function registerRewindCommand(ctx: Context, snapshots: SnapshotStore): void {
  ctx.commands.register({
    ...
    handler: async ({ agent, rawInput, signal }): Promise<CommandResult> => {
      let parsed: ReturnType<typeof parseRewindArgs>
      try {
