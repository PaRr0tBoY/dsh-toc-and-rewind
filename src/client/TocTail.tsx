 ...
 * rounded directory panel listing every request; it closes on blur, mouse
 * leave, or Escape.
 ...
 * @param props - injected controller resolver; `useSessions`/`t` ride the standard seats.
 * @returns the rail, or null while hidden (no chat view, narrow column, no session).
 */
export function TocTail({ controllerFor, useSessions, t }: TocTailProps): JSX.Element | null {
        ...
        <div className={css.directory} role="list" aria-label={t('directory.aria')}>
          {entries.map((entry, index) => {
            ...
            return (
              ...
              >
                <span className={css.directoryIndex}>{index + 1}</span>
